...
[legendStr,plotTitle,plotXlabel,plotYlabel,lineWidth,colors,lineStyles,...
    ...
    useLines,fillFace,errors,errorStyle,errorColors,errorFill,labelLines,labelLoc,labelRotate,LegendFontSize,axisFontSize,axisFontSize2] = ...
    myProcessOptions(options,'legend',[],'title',[],'xlabel',[],'ylabel',[],...
    'lineWidth',6,'colors',[],'lineStyles',[],...
    'markerSize',3,'markers',[],'markerSpacing',[],...
    'xlimits',[],'ylimits',[],...
    'logScale',0,'legendLoc','Best','useLines',1,'fillFace',1,...
    'errors',[],'errorStyle',{'--'},'errorColors',[],'errorFill',0,'labelLines',1,'labelLoc',2,'labelRotate',1,'LegendFontSize',19,'axisFontSize',23,'axisFontSize2',23);

 
% Ganzhao's parameters
% [colors] = loadcolor;
% lineStyles=loadlinestyples;
% markers=loadmarkers;
% errorColors=loaderrorcolor;
% markerSize=12;
% markerSpacing=loadmarkerSpacing;
% labelLines = 1;
% labelRotate = 1;
 ;
...
if ~isempty(errors)
    ...
    for i = 1:nLines
        if errorFill == 0
            ...
            hE = fill([x{i}(:);flipud(x{i}(:))],...
                [errors{i,1}(:);flipud(errors{i,2}(:))],...
                errorColors(i,:),'LineStyle','none');
        end
    end
...
set(gca,'FontName','AvantGarde','FontWeight','normal','FontSize',axisFontSize);
...
if ~isempty(plotXlabel) || ~isempty(plotYlabel)
    ...
    set([h1 h2],'FontName','AvantGarde','FontSize',axisFontSize2,'FontWeight','normal');
end
...
%             stringVal = [repmat(' ',1,markerSize/4),legendStr{i}];
        ...
        set(h,'FontSize',LegendFontSize,'FontWeight','normal');
        set(h,'Location','Best');
...
pcolor.crimson  = [192,52,148]/256;   pcolor.crimson2 = [245,147,202]/256;
...
pcolor.pink     = [255,180,180]/256;
pcolor.def      = [256,256,230]/256;

colors  = [];
colors{1}=pcolor.red;
colors{2}=pcolor.blue;
colors{3}=pcolor.green;
colors{4}=pcolor.yellow;
colors{5}=pcolor.gray;
colors{6}=pcolor.purple;
colors{7}=pcolor.pink;
colors{8}=pcolor.orange;
colors{9}=pcolor.crimson;
